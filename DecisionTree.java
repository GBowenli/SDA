...
public class DecisionTree implements Serializable {
	...
	class DTNode implements Serializable{
		...
		// this method takes in a datalist (ArrayList of type datum) and a Classification (int) and returns
		...
		DTNode fillDTNode(ArrayList<Datum> datalist) {
		    ...
		    double[] bestValues; // best attribute and threshold
		    
		    ArrayList<Datum> datalist1 = new ArrayList<Datum>();
		    ArrayList<Datum> datalist2 = new ArrayList<Datum>();
			...
			if (allSameLabel) {
			    ...
			    bestValues = findBestSplit(datalist);		    
			    newNode.attribute = (int)bestValues[0];
			    newNode.threshold = bestValues[1];
			    newNode.leaf = false;
			    
			    for (Datum data : datalist) {
				if (data.x[(int)bestValues[0]] < bestValues[1]) {
				    datalist1.add(data);
				} else {
				    datalist2.add(data);
				}
			    }
			    newNode.left = fillDTNode(datalist1);
			    newNode.right = fillDTNode(datalist2);
			    return newNode;
			}
		    } else {
			newNode.leaf = true;
		...
		double[] findBestSplit(ArrayList<Datum> datalist) {
		    ...
		    for (int i = 0; i < 2; i++) {
			for (Datum data : datalist) {
			    currentThreshold = data.x[i];
			    
			    for (Datum temp : datalist) {
				if (temp.x[i] < currentThreshold) {
				    datalist1.add(temp);
				} else {
				    datalist2.add(temp);
				}
			    ...
			    currentAvgEntropy = ((double)datalist1.size()) / datalist.size() * calcEntropy(datalist1) + ((double)datalist2.size()) / datalist.size() * calcEntropy(datalist2); 
			    if (bestAvgEntropy > currentAvgEntropy) {
				bestAvgEntropy = currentAvgEntropy;
				bestValues[0] = i;
				bestValues[1] = currentThreshold;
			    }
			    datalist1.clear();
			    datalist2.clear();
			}
		...
		public boolean equals(Object dt2) // NOT SURE IF THIS IS 100% CORRECT
		{   
		    if (this.leaf == false && ((DTNode)dt2).leaf == false) {
			if (this.threshold == ((DTNode)dt2).threshold && this.attribute == ((DTNode)dt2).attribute) {
			    return (this.left.equals(((DTNode)dt2).left) && this.right.equals(((DTNode)dt2).right));
			} else {
			    return false;
			}
		    } else if (this.leaf == true && ((DTNode)dt2).leaf == true) {
